...
在创建或执行 OpenSpec 规划 artifact 时，把架构评估作为设计质量的一部分。此指导适用于不同技术栈和不同类型的变更，不针对任何单一场景。

## 评估深度与变更显著性成正比

先判断变更的架构显著性，再决定评估深度：

- **显著变更**（跨模块、修改共享契约、新增依赖/服务/存储/协议、涉及迁移/安全/性能/兼容性）：完整应用本指导的各节。
- **简单变更**（在既有模式内的局部修改，无新抽象、无新决策）：只需确认复用现状和影响范围，一两句话即可，不要为凑格式展开分析。
